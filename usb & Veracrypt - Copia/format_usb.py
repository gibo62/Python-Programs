...
for i, drive_vera in enumerate(list_difference):
    ...
    password = base64.b64encode(hashlib.sha512(str(usb_id[i]).encode('UTF-8')).hexdigest().encode("utf-8"))
    print (password)
    print(f'"C:\\Program Files\\VeraCrypt\\VeraCrypt Format.exe" /create "{list_difference[i]}\\nomefile.vc" /filesystem exfat /size "{capacity[i]}" /password {str(password[:128],"utf-8")} /quick /fastcreatefile /silent')
